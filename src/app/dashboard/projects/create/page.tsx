...
import { ArrowLeft, Save, UploadCloud, AlertTriangle, MapPin } from "lucide-react";
...
export default function AddProjectPage() {
    ...
    useEffect(() => {
        if (session?.user?.name) {
            sanityClient.fetch(`*[_type == "companyUser" && company_name == $name][0]{ _id }`, { name: session.user.name })
                .then(res => setCompanyId(res?._id || ""));
        }
    }, [session]);
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (!companyId) return alert("No se pudo identificar tu ID de empresa. Intenta nuevamente.");
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    <h2 className="text-2xl font-black text-slate-800">Publicar Obra Realizada</h2>
                    <p className="text-slate-500 text-sm">Agrega fotos reales de tus instalaciones terminadas.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white border text-sm border-slate-200 rounded-2xl shadow-sm p-8 space-y-8">
                <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="md:col-span-2">
                            ...
                            <input required name="title" type="text" placeholder="Ej. Casa Mediterránea en Parcelación El Sauce" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#3200C1] focus:outline-none" />
                        </div>
                        <div className="md:col-span-2">
                            <label className="block text-slate-700 font-bold mb-2 uppercase text-[10px]">Descripción (Opcional)</label>
                            <textarea name="description" rows={3} placeholder="Breve testimonio o detalles del cliente..." className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#3200C1] focus:outline-none resize-none"></textarea>
                        </div>
                        <div>
                            <label className="block text-slate-700 font-bold mb-2 uppercase text-[10px]">Ubicación (Ciudad/Condominio)</label>
                            <input required name="location_name" type="text" placeholder="Ej. Curacaví, RM" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#3200C1] focus:outline-none" />
                        </div>
                        <div>
                            <label className="block text-slate-700 font-bold mb-2 uppercase text-[10px]">Fecha de Entrega</label>
                            <input name="completion_date" type="date" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#3200C1] focus:outline-none" />
                        </div>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input name="lat" type="number" step="0.000001" placeholder="Ej. -33.456" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#3200C1] focus:outline-none bg-slate-50" />
                        </div>
                        <div>
                            <label className="block text-slate-700 font-bold mb-2 uppercase text-[10px]">Longitud</label>
                            <input name="lng" type="number" step="0.000001" placeholder="Ej. -70.789" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:border-[#3200C1] focus:outline-none bg-slate-50" />
                        </div>
                    </div>
                </section>
                ...
                <section>
                    <h3 className="text-lg font-bold text-[#3200C1] mb-6">Fotos Reales del Terreno</h3>
                    <div className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-colors ${
                        ...
                        <p className="text-xs font-bold text-slate-400 mb-6 uppercase">Máximo <span className="text-[#3200C1]">3 MB</span> por foto.</p>
                ...
                <div className="pt-6 border-t border-slate-100 flex justify-end gap-4">
                    ...
                    >
                        {isPending ? 'Guardando...' : <><Save className="w-5 h-5" /> Publicar Obra</>}
                    </button>
                </div>
            </form>
        </div>
