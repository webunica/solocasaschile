...
export default function EditPostForm({ post }: { post: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await editPostAction(post._id, formData);
    ...
    if (status === "success") {
        return (
            <div className="bg-green-50 text-green-700 p-8 rounded-2xl flex flex-col items-center justify-center border border-green-200">
                ...
                <h3 className="text-xl font-bold">¡Publicación Actualizada!</h3>
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 pb-12">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
                ...
                {post.coverImageUrl && (
                    <div className="mb-4">
                        <span className="block text-xs font-black text-slate-500 uppercase mb-2">Imagen Actual</span>
                        <img src={post.coverImageUrl} alt="Actual" className="w-48 h-auto rounded-lg shadow-sm border border-slate-200" />
                    </div>
                )}
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="col-span-2">
                        <label className="block text-xs font-black text-slate-500 uppercase mb-2">Reemplazar Archivo de Imagen</label>
                        <input
                            type="file"
                            accept="image/*"
            ...
            <div className="flex justify-end gap-4 pb-8">
                ...
                >
                    {status === "loading" ? "Actualizando..." : "Actualizar Artículo"}
