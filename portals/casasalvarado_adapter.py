...
class CasasAlvaradoAdapter(BaseSiteAdapter):
    def __init__(self):
        super().__init__("https://casasalvarado.cl")
    ...
    def discover_model_urls(self) -> List[str]:
        ...
        for a in soup.find_all('a', href=True):
            href = a['href']
            # product pages usually end with .html
            if '.html' in href and href.startswith(self.base_url):
                urls.add(href)
    ...
    def parse_model_page(self, url: str) -> Optional[ModelSchema]:
        ...
        model_name = title_tag.text.strip().title() if title_tag else url.strip('/').split('/')[-1].replace('.html', '').replace('-', ' ').title()

        # Superficie 
        surface = None
        m2_match = re.search(r'(\d+(?:,\d+|\.\d+)?)\s*mt2', text_content, re.IGNORECASE)
        if m2_match:
        ...
        filtered_imgs = [
             ...
             and '/img/' in src and src.endswith('.jpg')
        ]
        ...
        return ModelSchema(
            ...
            description="Variedad en diseños, constructora enfocada en proyectos de madera.",
