...
class CasasAmericaAdapter(BaseSiteAdapter):
    def __init__(self):
        super().__init__("https://www.casasamerica.cl")
    ...
    def discover_model_urls(self) -> List[str]:
        ...
        for a in soup.find_all('a', href=True):
            href = a['href']
            # product pages usually contain /producto/ or similar, and are within the domain
            if self.base_url in href and len(href.split('/')) > 4 and ('producto' in href or 'modelo' in href or 'casa' in href):
                if 'contacto' not in href and 'nosotros' not in href:
                    urls.add(href)
                
        # let's try getting categories first
        category_urls = [a['href'] for a in soup.find_all('a', href=True) if 'categoria' in a['href'] and self.base_url in a['href']]
        for c in category_urls:
            csoup = self.get_soup(c)
            if csoup:
    ...
    def parse_model_page(self, url: str) -> Optional[ModelSchema]:
        ...
        filtered_imgs = [
             ...
             and not any(x in src.lower() for x in ['logo', 'icon', 'whatsapp', 'svg', 'data:', 'fb-', 'instagram', 'avatar'])
             and ('wp-content' in src or src.endswith('.jpg'))
        ]
        ...
        return ModelSchema(
            ...
            description="Fabricante líder en calidad para construcción modular y madera.",
